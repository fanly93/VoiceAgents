...
def test_dashscope_diagnostics_use_registry_without_openai_checks() -> None:
    ...
    assert check_by_name(diagnostics, "provider_supported").status == "pass"
    assert check_by_name(diagnostics, "provider_model").status == "pass"
    assert check_by_name(diagnostics, "dashscope_api_key").status == "pass"
    assert check_by_name(diagnostics, "dashscope_model").status == "pass"
    assert check_by_name(diagnostics, "dashscope_base_url").status == "pass"
    assert check_by_name(diagnostics, "dashscope_connection_mode").status == "pass"
    assert "openai_model" not in check_names
    assert "openai_api_key" not in check_names
    assert "dashscope-secret" not in diagnostics.model_dump_json()


def test_dashscope_diagnostics_fail_when_api_key_is_missing() -> None:
    diagnostics = build_realtime_dev_diagnostics(
        {
            "VOICEAGENTS_REALTIME_PROVIDER": "dashscope_realtime",
    ...
    assert diagnostics.overall_status == "fail"
    check = check_by_name(diagnostics, "dashscope_api_key")
    assert check.status == "fail"
    assert "DASHSCOPE_API_KEY" in check.remediation
